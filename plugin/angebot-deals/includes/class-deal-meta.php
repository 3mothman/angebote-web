...
final class Angebot_Deals_Deal_Meta
{
    ...
    public static function render_meta_box(WP_Post $post): void
    {
        ...
        <div class="angebot-meta-grid">
            <div>
                <label for="original_price"><?php esc_html_e('Originalpreis (€)', 'angebot-deals'); ?></label>
            ...
            <div>
                <label for="deal_price"><?php esc_html_e('Deal-Preis (€)', 'angebot-deals'); ?></label>
            ...
            <div>
                <label for="discount_percent"><?php esc_html_e('Rabatt % (optional, sonst auto)', 'angebot-deals'); ?></label>
            ...
            <div>
                <label for="quantity"><?php esc_html_e('Kontingent / verfügbare Anzahl', 'angebot-deals'); ?></label>
                <input type="number" min="0" id="quantity" name="angebot_meta[quantity]" value="<?php echo esc_attr((string) $data['quantity']); ?>">
            </div>
            <div>
                <label for="merchant_name"><?php esc_html_e('Anbieter-Name', 'angebot-deals'); ?></label>
                <input type="text" id="merchant_name" name="angebot_meta[merchant_name]" value="<?php echo esc_attr($data['merchant_name']); ?>">
            </div>
            <div>
                <label for="merchant_user_id"><?php esc_html_e('Merchant-Benutzer', 'angebot-deals'); ?></label>
                <select id="merchant_user_id" name="angebot_meta[merchant_user_id]">
                    <option value="0"><?php esc_html_e('— Keiner —', 'angebot-deals'); ?></option>
            ...
            <div>
                <label for="location_label"><?php esc_html_e('Standort-Label (Anzeige)', 'angebot-deals'); ?></label>
                <input type="text" id="location_label" name="angebot_meta[location_label]" value="<?php echo esc_attr($data['location_label']); ?>" placeholder="z. B. Berlin-Mitte">
            </div>
            <div>
                <label for="voucher_expires"><?php esc_html_e('Gutschein-Ablaufdatum', 'angebot-deals'); ?></label>
                <input type="date" id="voucher_expires" name="angebot_meta[voucher_expires]" value="<?php echo esc_attr($data['voucher_expires']); ?>">
            </div>
            <div class="full">
                <label for="highlights"><?php esc_html_e('Highlights (eine Zeile pro Punkt)', 'angebot-deals'); ?></label>
                <textarea id="highlights" name="angebot_meta[highlights]" rows="4"><?php echo esc_textarea($data['highlights']); ?></textarea>
            </div>
            <div class="full">
                <label for="fine_print"><?php esc_html_e('Kleingedrucktes / Bedingungen', 'angebot-deals'); ?></label>
            ...
            <div>
                <label>
                    ...
                    <?php esc_html_e('Als Highlight auf Startseite', 'angebot-deals'); ?>
                </label>
            </div>
            <div>
                <p><strong><?php esc_html_e('Verkauft:', 'angebot-deals'); ?></strong> <?php echo esc_html((string) (int) $data['sold_count']); ?></p>
                <p><strong><?php esc_html_e('WC-Produkt-ID:', 'angebot-deals'); ?></strong> <?php echo esc_html((string) (int) $data['product_id']); ?></p>
            </div>
        </div>
        <?php
    }
    ...
    public static function columns(array $columns): array
    {
        ...
        foreach ($columns as $key => $label) {
            $new[$key] = $label;
            if ($key === 'title') {
                $new['deal_price'] = __('Preis', 'angebot-deals');
                $new['deal_stock'] = __('Kontingent', 'angebot-deals');
                $new['deal_merchant'] = __('Anbieter', 'angebot-deals');
            }
        }
        return $new;
    }
        ...
        switch ($column) {
            ...
            case 'deal_stock':
                ...
                echo $qty > 0
                    ...
                    : esc_html__('Unbegrenzt', 'angebot-deals');
                break;
            case 'deal_merchant':
