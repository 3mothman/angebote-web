...
<head>
    ...
    <title><?php esc_html_e('Gutschein', 'angebot-deals'); ?> — <?php bloginfo('name'); ?></title>
    <?php wp_head(); ?>
</head>
<body class="angebot-voucher-page">
<main class="angebot-voucher-shell">
    <?php if (!$voucher) : ?>
        <h1><?php esc_html_e('Gutschein nicht gefunden', 'angebot-deals'); ?></h1>
    <?php else :
        ...
        <?php if ($voucher->expires_at) : ?>
            <p><?php printf(esc_html__('Gültig bis %s', 'angebot-deals'), esc_html(date_i18n('d.m.Y', strtotime($voucher->expires_at)))); ?></p>
        <?php endif; ?>
        ...
        <?php if ($can_redeem && $voucher->status === Angebot_Deals_Voucher::STATUS_ACTIVE) : ?>
            <button type="button" class="angebot-btn angebot-btn--primary" id="angebot-redeem-from-qr" data-id="<?php echo esc_attr((string) $voucher->id); ?>">
                <?php esc_html_e('Als eingelöst markieren', 'angebot-deals'); ?>
            </button>
            ...
            document.getElementById('angebot-redeem-from-qr')?.addEventListener('click', async function () {
                ...
                msg.hidden = false;
                msg.textContent = json.data?.message || json.data?.message || (json.success ? 'OK' : 'Fehler');
                if (json.success) location.reload();
            });
            </script>
        <?php elseif (!$can_redeem) : ?>
            <p class="angebot-hint"><?php esc_html_e('Kunde: Zeig diesen Screen dem Anbieter. Anbieter: Bitte einloggen, um einzulösen.', 'angebot-deals'); ?></p>
        <?php endif; ?>
    <?php endif; ?>
</main>
<?php wp_footer(); ?>
