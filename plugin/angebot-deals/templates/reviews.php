...
<section class="angebot-reviews" id="angebot-reviews" data-deal="<?php echo esc_attr((string) $deal_id); ?>">
    <header class="angebot-reviews__header">
        <h2><?php esc_html_e('Bewertungen', 'angebot-deals'); ?></h2>
        <?php if ($avg > 0) : ?>
    ...
    <div class="angebot-reviews__list">
        <?php if (!$reviews) : ?>
            <p><?php esc_html_e('Noch keine Bewertungen — sei der Erste!', 'angebot-deals'); ?></p>
        <?php else : foreach ($reviews as $review) : ?>
            <article class="angebot-review">
                <div class="angebot-review__meta">
                    ...
                    <span class="angebot-stars" aria-label="<?php echo esc_attr((string) $review->rating); ?> Sterne">
    ...
    <form class="angebot-review-form" id="angebot-review-form">
        <h3><?php esc_html_e('Bewertung schreiben', 'angebot-deals'); ?></h3>
        <input type="hidden" name="deal_id" value="<?php echo esc_attr((string) $deal_id); ?>">
        <label>
            <span><?php esc_html_e('Sterne', 'angebot-deals'); ?></span>
        ...
        <label>
            <span><?php esc_html_e('E-Mail', 'angebot-deals'); ?></span>
            <input type="email" name="author_email" value="<?php echo esc_attr(wp_get_current_user()->user_email ?? ''); ?>">
        </label>
        <label>
            <span><?php esc_html_e('Titel', 'angebot-deals'); ?></span>
            <input type="text" name="title">
        </label>
        <label>
            <span><?php esc_html_e('Deine Erfahrung', 'angebot-deals'); ?></span>
            <textarea name="content" rows="4" required></textarea>
        </label>
        <button type="submit" class="angebot-btn angebot-btn--primary"><?php esc_html_e('Absenden', 'angebot-deals'); ?></button>
